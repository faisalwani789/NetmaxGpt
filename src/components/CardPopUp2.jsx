...
const CardPopUp2 = ({ showPopUp, setShowPopUp, title, img, description,id }) => {
  ...
  return (
    <div className='fixed inset-0 bg-black/60 z-60 ' onClick={ handleSetShowPopUp }>
      ...
      <div className={`pb-20 bg-black rounded-xl shadow-2xl overflow-y-scroll transform -translate-x-1/2 -translate-y-1/2 fixed w-90 h-100 ${!closing?'animate-growPop':'animate-shrinkPop'}  z-100 hide-scrollbar  `} style={{ top: cardPosition.top, left: cardPosition.left, "--final-top": `${10}px`, "--final-width": `${850}px `, "--final-height": `${768}px` ,"--initial-top":`${cardPosition.top}px`,  "--initial-left":`${cardPosition.left}px` }} onClick={(e) => e.stopPropagation()}>
        ...
        {/* title/overlay*/}
        <div className='ml-10 absolute bottom-90'>
